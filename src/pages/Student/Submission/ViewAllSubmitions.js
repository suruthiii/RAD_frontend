...
export default function All_submition() {
  ...
  const delete_submition = (id) => {
    axios
      .delete("http://localhost:8070/submition/delete/" + id)
      .then(() => {
  ...
  return (
    <div>
      ...
      <div className="container">
        <Link to="/AddAubmission">Add New</Link>
          ...
          <tbody className="body">
            {submition.map((names, index) => (
              <tr key={index}>
                ...
                <td>
                  <Link className="link" to={`/UpdateSubmission/${names._id}`}>
                    update
                  </Link>
                  &nbsp;&nbsp;&nbsp;
                  <button
                    className="delete_button"
                    onClick={() => delete_submition(names._id)}
                  >
                    delete
                  </button>
                </td>
